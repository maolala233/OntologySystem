...
class OllamaEmbed(Base):
    ...
    def __init__(self, key, model_name, **kwargs):
        ...
        self._embedding_dim = int(os.environ.get("OLLAMA_EMBED_FALLBACK_DIM", 1024))

    @staticmethod
    def _sanitize_text(text: str) -> str:
        """Remove characters that may cause Ollama models to produce NaN embeddings."""
        if text is None:
            return ""
        text = str(text)
    ...
    def _clean_embedding(self, embedding: list | None) -> list | None:
        ...
        for v in embedding:
            ...
            cleaned.append(fv)
        if cleaned:
            self._embedding_dim = len(cleaned)
        return cleaned

    def _fallback_embedding(self) -> list:
        return [0.0] * self._embedding_dim

    @staticmethod
    def _is_ollama_nan_error(err: Exception) -> bool:
        msg = str(err).lower()
        return "nan" in msg or "failed to encode response" in msg or "unsupported value" in msg

    def _encode_single(self, txt: str, retries: int = 2) -> list:
        """Encode a single text; if Ollama returns NaN, degrade to a zero vector instead of aborting KG build."""
        sanitized = self._sanitize_text(txt)
        candidates = [sanitized]
        if len(sanitized) > 2048:
            candidates.append(sanitized[:2048])
        if len(sanitized) > 512:
            candidates.append(sanitized[:512])
        candidates.append("empty")

        last_err = None
        for candidate in candidates[:retries + 2]:
            try:
                res = self.client.embeddings(
                    prompt=candidate, model=self.model_name,
                    options={"use_mmap": True}, keep_alive=self.keep_alive
                )
                embedding = self._clean_embedding(res.get("embedding"))
                if embedding:
                    return embedding
            except Exception as e:
                last_err = e
                if self._is_ollama_nan_error(e):
                    logger.warning(
                        "Ollama embedding returned NaN/invalid JSON, retrying with sanitized text. text=%r fallback_dim=%s error=%s",
                        candidate[:100], self._embedding_dim, e,
                    )
                    continue
                raise
        ...
        if last_err:
            logger.warning(
                "Ollama embedding failed after retries, using zero-vector fallback. text=%r fallback_dim=%s error=%s",
                sanitized[:100], self._embedding_dim, last_err,
            )
    ...
    def encode(self, texts: list):
        ...
        for txt in texts:
            ...
            try:
                embedding = self._encode_single(txt)
                arr.append(embedding)
            except Exception as _e:
                log_exception(_e, {"text": txt[:100]})
                raise Exception(f"Error: {_e}")
            tks_num += 128
        max_dim = max((len(e) for e in arr if e), default=self._embedding_dim)
        self._embedding_dim = max_dim
        arr = [e if len(e) == max_dim else e[:max_dim] + [0.0] * (max_dim - len(e)) for e in arr]
        return np.array(arr, dtype=float), tks_num
    ...
    def encode_queries(self, text):
        ...
        try:
            ...
            return np.array(embedding, dtype=float), 128
        except Exception as _e:
            log_exception(_e, {"text": text[:100]})
            raise Exception(f"Error: {_e}")
