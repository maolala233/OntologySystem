...
class KGSearch(Dealer):
    ...
    def get_relevant_ents_by_names(self, names, filters, idxnms, kb_ids, N=56):
        if not names:
            return {}
        filters = deepcopy(filters)
        filters["knowledge_graph_kwd"] = "entity"
        filters["entity_kwd.keyword"] = list({str(name).upper() for name in names if name})
        es_res = self.dataStore.search(["content_with_weight", "entity_kwd", "rank_flt", "n_hop_with_weight"], [], filters, [],
                                       OrderByExpr(), 0, N,
                                       idxnms, kb_ids)
        ents = self._ent_info_from_(es_res, 0)
        for ent in ents.values():
            ent["sim"] = max(ent.get("sim", 0), 1.0)
        return ents
    ...
    def get_relevant_ents_by_types(self, types, filters, idxnms, kb_ids, N=56):
        ...
        filters = deepcopy(filters)
        filters["knowledge_graph_kwd"] = "entity"
        # entity_type_kwd 是 text 类型，term 查询需使用 keyword 子字段
        filters["entity_type_kwd.keyword"] = types
        ordr = OrderByExpr()
        ordr.desc("rank_flt")
    ...
    def get_relation_by_entities(self, from_entity, to_entity, filters, idxnms, kb_ids):
        filters = deepcopy(filters)
        filters["knowledge_graph_kwd"] = "relation"
        filters["from_entity_kwd.keyword"] = from_entity
        filters["to_entity_kwd.keyword"] = to_entity
        es_res = self.dataStore.search(["content_with_weight", "weight_int"], [], filters, [], OrderByExpr(), 0, 1,
                                       idxnms, kb_ids)
        rels = self.dataStore.get_fields(es_res, ["content_with_weight", "weight_int"])
        if rels:
            row = next(iter(rels.values()))
            return {"description": row.get("content_with_weight", ""), "weight": row.get("weight_int", 1)}

        filters["from_entity_kwd.keyword"] = to_entity
        filters["to_entity_kwd.keyword"] = from_entity
        es_res = self.dataStore.search(["content_with_weight", "weight_int"], [], filters, [], OrderByExpr(), 0, 1,
                                       idxnms, kb_ids)
        rels = self.dataStore.get_fields(es_res, ["content_with_weight", "weight_int"])
        if rels:
    ...
    async def retrieval(self, question: str,
        ...
        try:
            ...
            ty_kwds = [str(t).upper() for t in ty_kwds if t]
            ents = [str(e).strip() for e in ents if e]
        ...
        ents_from_query = self.get_relevant_ents_by_names(ents, filters, idxnms, kb_ids, ent_topn * 4)
        vector_ents = await self.get_relevant_ents_by_keywords(ents, filters, idxnms, kb_ids, emb_mdl, ent_sim_threshold)
        ents_from_query.update(vector_ents)
        if not ents_from_query and ents != [qst]:
            ents_from_query = await self.get_relevant_ents_by_keywords([qst], filters, idxnms, kb_ids, emb_mdl, min(ent_sim_threshold, 0.1))
        ents_from_types = self.get_relevant_ents_by_types(ty_kwds, filters, idxnms, kb_ids, 10000)
        rels_from_txt = await self.get_relevant_relations_by_txt(qst, filters, idxnms, kb_ids, emb_mdl, min(rel_sim_threshold, 0.1))
        ...
        for ent, info in ents_from_types.items():
            if ent in ents_from_query:
                ents_from_query[ent]["sim"] *= 2
            elif ent in {str(e).upper() for e in ents}:
                info["sim"] = max(info.get("sim", 0), 0.8)
                ents_from_query[ent] = info
        ...
        for (f, t), rel in rels_from_txt:
            if not rel.get("description"):
                for tid in tenant_ids:
                    ...
                    rela = None
                if not rela:
                    rela = self.get_relation_by_entities(f, t, filters, idxnms, kb_ids)
                if not rela:
                    continue
